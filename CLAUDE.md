...
- **小窓モード(`js/mini.js`、2026-09-25)**: ヘッダーの「小窓」でDocument PiPを開き、全アンサンブルのMIDIカードを
  ...
  動いていて壊れるため。ドラッグはChrome/Edgeの`DownloadURL`(dragstartで「MIME:ファイル名:blob URL」)で、全トラックか
  パート1つずつ(`buildSmf(card, part)`)。MIDIのパネルにも同じチップがある。**Cubaseへ直接ドロップできるかは実機未確認**
  (入らなければデスクトップ経由)。発言のレシピの小窓と同時には開けない(PiPは1ページに1枚)
