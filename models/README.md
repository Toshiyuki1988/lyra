...
| id | 名前 | 由来 | 何をするか |
|---|---|---|---|
| `chords` | 和音 | 楽典 | コードを伴奏の型(伸ばす・刻む・裏拍・8分・分散・アルベルティ・ジャズのコンピング・チャールストン・食い)で鳴らす。`hits` にリズム譜(X=強 x=普通 o=弱 .=休み -=伸ばす)を書くと、その位置と強さで打つ(ずれたアクセントの連打、ボサノヴァの刻みなど)。積み方は声部進行の近さで選ぶ。`parallel` はプラーニング、`rootless` はジャズのルートレス・ボイシング。コード進行が無い時は音階の度数から和音を作る |
| `bass` | ベース | 楽典 | ルート・ルートと5度・オクターブ・持続・ボサノヴァ・ウォーキング(拍の頭に根音、途中はコードの音か音階の段、コードが変わる直前は半音・全音・5度から寄る。たまに3連の跳ね) |
| `bebop` | アドリブ線 | ジャズ | コードスケールの上の8分の線。強拍にコードの音、コードが変わる直前は半音のアプローチか囲み込みで次のコードの3度・7度へ、3連のターン、フレーズの終わりはコードの音で息継ぎ。緊張が高いほど長く高く |
...
| ジャズ | `jazz_bebop` | ジャズ・ビバップ | chords(ルートレス・ジャズのコンピング)・bass(ウォーキング)・bebop・line・drums | テーマ(line、反芻)とソロ(bebop)を区間で鳴らし分け |
...
## 5.2 プロンプトカード(テキストカードの属性、2026-09-26)

ユーザー要望「テキストカードの属性に『プロンプト』を作って。ざっくりとした僕の要望をMIDI生成に最適な文章に直す。『課題』『気づき』から変換可能」。

- 気づき・課題カードの右下「→プロンプト」で、Geminiを1回呼んで要望を書き直す(`refinePrompt()`、compose.js)。観点は、情景と時間の流れ・
  感情の起伏と頂点・テンポ・拍子とリズム・和声と音階・質感と層・形式と長さ・避けたいこと(箇条書き6〜9行、350字以内)。
  「〇〇の曲みたいに」は技法・雰囲気・リズムの言葉に置き換えさせる。合うモデル・小節数・ゲージ・選んだ理由も出させる
- 同じカードがプロンプトカードになる(位置・線はそのまま)。`card.text` は元の要望、`card.prompt` は整えた文(Editで直せる)、
  `model`・`bars`・`gauges`・`why`。上の「整える」で書き直し直す(手を入れた文も踏まえる)。左下で元の気づき・課題へ戻せ、
  要望を変えなければ次の「→プロンプト」はGeminiを呼ばない(`promptFrom`)
- 使われ方: プロンプトカード自身の「鳴らす」「ビート」、またはつないだ課題の「鳴らす」(プロンプトカードがあればソウル無しでも使える)。
  整えた文は、つないだカードの行として「最優先で従う」の印つきでGeminiに渡る(ensemble.js の `cardLine`)。おすすめのモデルはピッカーに
  「おすすめ」の印、小節数・ゲージはダイアログの初期値になる(★評価の平均より優先)
