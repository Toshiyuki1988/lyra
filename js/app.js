 ...
 * 美学は「美学的特徴」(音に言い換えず、視覚・質感などを残す)と「音への翻案」(音楽にするなら)の2本立て。
 * 著作権ゲート(ハンドオフ8節)どおり、どの区分でも資料の文章は写させず自分の言葉で要約させる(「そのまま」とは書かない)。
 */
const READING_FIELDS = {
  ...
  genre: {
    effect: '特徴', effectHint: 'そのジャンルらしさを作る要素', effectPrompt: '特徴。資料が述べているそのジャンルらしさの要素(音・リズム・和声・編成に限らず、文化・時代・服装・映像なども資料にあれば含める)',
  ...
  aesthetic: {
    ...
    effectPrompt: '美学的特徴。資料が述べている特徴(視覚・色・質感・モチーフ・雰囲気・時代・文化など)を、音や音楽に言い換えずに書く。ただし資料の文章は写さず、自分の言葉で短く要約する',
